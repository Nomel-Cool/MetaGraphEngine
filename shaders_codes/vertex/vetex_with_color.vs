...
layout (location = 1) in vec4 aColor;// pos 1 is for color data.
layout (location = 2) in vec3 aNormVec; // 该顶点在X分量上的法向量（Damn，无非即使(1,0,0)或(-1,0,0)嘛)
layout (location = 3) in vec2 aTexCoord;// pos 2 is for texture ccordinates.
